...
for VAL in ${LIST}
do
        PROF=`tmsh -c "cd /; show /ltm virtual ${VAL} profiles" 2> /dev/null | grep -B 1 " Ltm::ClientSSL Profile:" | cut -d: -f4 | grep -i "[a-z]" | sed s'/ //'g| sort -u`
        test -n "${PROF}" 2>&- && {
                for PCRT in ${PROF}
                do
                        CERT=`tmsh -c "cd /; list /ltm profile client-ssl ${PCRT}" |  awk '$1 == "cert" {print $2}' 2> /dev/null | sort -u`
                        test -n "${CERT}" 2>&- && {
                        	echo "${VAL},${PCRT},${CERT}"
                        }
                done
